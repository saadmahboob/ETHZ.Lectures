...
function [] = XCT_EXERCISE3()
    ...
    Ne              = Ia*1e-3/e;            % #electrons          [1/s]   
    N0              = yield*Ua*Ne*dt;       % #photons            []    
        
    if Ua==50  idx = 1; end                 % set index 
    if Ua==100 idx = 2; end
    
    matrix          = 256;                  % image matrix [1pix=1mm]
    R               = 4;                    % radial undersampling factor 
    ...
    for phi=projection_angles
        ...
        % Let the heart beat 
        % ----------------------------------------------------------------
        phantom.ellipse(8,4) = heart_axis+5*sin(2*pi*dt*idx);
        ...
        % ----------------------------------------------------------------
        % Calculate signal using Beer-Lambert 
        % ----------------------------------------------------------------
        N = N0*exp(-phantom.sino(:,idx));
...
function [projection] = AddNoise(projection)
    ...
    % Add Poisson noise 
    % --------------------------------------------------------------------
    projection = poissrnd(projection,size(projection));
...
function phantom = ApplyGaussianFilter(phantom,factor)
    ...
    % --------------------------------------------------------------------
    % Define sigma based on full-width-at-half-maximum (FWHM)
    % --------------------------------------------------------------------   
    sigma   = matrix/factor/(2*sqrt(2*log(2)));
...
%=========================================================================
function [snr] = CalcSNR(phantom,k)
    ...
    % --------------------------------------------------------------------    
    snr = mean(phantom.img(i))/std(phantom.img(i)); 
