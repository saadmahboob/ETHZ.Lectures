...
function [] = NUC_EXERCISE2()
    ...
    for i = 1:10
        ...
        % ----------------------------------------------------------------
        % Task 2.2 Define fit function using 
        % ct_noise(t) = int(cp_noise(tau)*(h_t(t-tau))dtau  
        % use filter() to perform the convolution
        % ----------------------------------------------------------------
        fun = @(k,t)filter(cp_noise,1,k(1).*exp(-t.*(k(2)+k(3))) + ...
                    (k(1).*k(3))./(k(2)+k(3))-(k(1).*k(3).*exp(-t.*(k(2)+k(3))))./(k(2)+k(3)));
    ...
    % --------------------------------------------------------------------
    % Optional - provide Coefficient of Variation (CoV)
    % --------------------------------------------------------------------
    fprintf('CoV   [k1 k2 k3]: %f %f %f\n',std(k_result,1,2)./mean(k_result,2)*100);
