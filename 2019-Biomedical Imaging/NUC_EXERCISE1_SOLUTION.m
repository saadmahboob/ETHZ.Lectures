...
function [] = NUC_EXERCISE1()
    ...
    % --------------------------------------------------------------------
    % Set tissue mass attenuation coefficients for 100 and 511 keV
    % (see: Table 4 in www.nist.gov/pml/data/xraycoef/)
    % --------------------------------------------------------------------
    mac_blood(1)    = 0.149;                % blood  @ 100 keV  [cm2/g]
    mac_blood(2)    = 0.096;                % blood  @ 511 keV  [cm2/g]
    mac_bone(1)     = 0.186;                % bone   @ 100 keV  [cm2/g]
    mac_bone(2)     = 0.090;                % bone   @ 511 keV  [cm2/g]
    mac_lung(1)     = 0.154;                % lung   @ 100 keV  [cm2/g]
    mac_lung(2)     = 0.096;                % lung   @ 511 keV  [cm2/g]
    mac_muscle(1)   = 0.169;                % muscle @ 100 keV  [cm2/g]
    mac_muscle(2)   = 0.096;                % muscle @ 511 keV  [cm2/g]
    mac_water(1)    = 0.171;                % water  @ 100 keV  [cm2/g]
    mac_water(2)    = 0.097;                % water  @ 511 keV  [cm2/g]
    ...
    % --------------------------------------------------------------------
    % Compute and display CT reconstruction (FBP)
    % --------------------------------------------------------------------
    filter           = CalcFilter(matrix+1);
    ...
    phantom.corr = (mue_bone(2)-mue_water(2))/(mue_bone(1)-mue_water(1))*phantom.ctfbp;
        
    % --------------------------------------------------------------------
    % Recompute CT sinogram from remapped CT image 
    % --------------------------------------------------------------------
    ...
    % --------------------------------------------------------------------
    % Recompute PET sinogram from reconstructed PET image
    % --------------------------------------------------------------------
    ...
    % --------------------------------------------------------------------
    % Task 1.3. Recompute PET image from attenuation-corrected sinogram
