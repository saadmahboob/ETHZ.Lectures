...
function [] = XCT_EXERCISE2()
    ...
    for phi=projection_angles
        ...
        title('Simple Backprojection'); xlabel('x'); ylabel('y'); drawnow;
    ...
    % --------------------------------------------------------------------
    [r]         = meshgrid(-fix(matrix/2):+fix(matrix/2),1); idx = 1; 
    [kx,ky]     = meshgrid(-fix(matrix/2):+fix(matrix/2));
    phantom.fft = zeros(matrix+1,matrix+1);
    
    for phi=projection_angles
    
        kx_(:,idx) = r*cos(phi/180*pi);
        ky_(:,idx) = r*sin(phi/180*pi);
        
        transformedprojection(:,idx) = R2U(phantom.sino(:,idx));
        
        idx = idx+1;
    end
   
    % Grid data to Cartesian matrix
    phantom.fft = griddata(kx_,ky_,transformedprojection,kx,ky);
...
function [filter] = CalcFilter(matrix)
    ...
    % --------------------------------------------------------------------
    % Compute low-pass filter according to 1/2*(1+cos())
    % --------------------------------------------------------------------
